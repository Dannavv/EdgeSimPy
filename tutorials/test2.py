...
class MovingUser(User):
    ...
    def _select_server(self):
        ...
        fog_cpu_threshold = 0.9  # CPU utilization threshold for offloading to cloud
        ...
        if d <= max_fog_distance and nearest_fog.cpu_util < fog_cpu_threshold:
    ...
    def step(self):
        ...
        # Users only generate requests based on a given probability
        if random.random() < self.request_probability:
            ...
            self.sla_violation_rate = self.sla_violations / self.total_requests if self.total_requests > 0 else 0.0
...
def simple_resource_management(parameters: dict):
    """A simple resource manager that clears server load periodically."""
    sim = parameters["model"]
    interval = parameters.get("interval", 10)

    if sim.schedule.steps % interval == 0:
        for server in EdgeServer.all():
            server.request_load = 0


def network_congestion_model(parameters: dict):
    """A simple user-defined function to simulate network congestion."""
    sim = parameters["model"]
    for link in NetworkLink.all():
        # Fluctuate delay by +/- 20% of its base delay
        fluctuation = random.uniform(-0.2, 0.2)
        link.delay = link.base_delay * (1 + fluctuation)
...
def build_infrastructure(sim: Simulator,
    ...
    # To connect a server directly to a switch (bypassing a base station),
    # we set its `network_switch` attribute. The `base_station` attribute
    # will be None, which is correct for a core cloud server.
    cloud.network_switch = switches[0]
    ...
    for i in range(len(switches) - 1):
        ...
        link.base_delay = link_delay  # Store base delay for congestion model
...
def run_experiment_markov_edgesimpy(
    ...
    # Set user-defined functions and resource management after simulator initialization
    # This allows passing the simulator instance "sim" to them via the parameters dictionary
    sim.resource_management_algorithm = simple_resource_management
    sim.resource_management_algorithm_parameters = {"model": sim, "interval": 10}
    sim.user_defined_functions = [network_congestion_model]
    sim.user_defined_functions_parameters = [{"model": sim}]
